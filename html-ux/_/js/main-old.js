...
function init() {
	...
	for (var i = 0; i < tableList.length; i++) {
		...
		// set up the buttons
		t.node = $('<option value="'+i+'">'+t.tableName+'</option>');
		$('#datasets select').append(t.node);
		
	}

	$("#datasets").on("change", 'select', function () {
...
function redraw() {
	...
	// draw the background
	context.fillStyle = 'rgba(103, 191, 173, 1)';
